  ...
  while (iterations < maxIterations) {
    ...
    if (response.stop_reason === "tool_use") {
      ...
      for (const toolUse of toolUseBlocks) {
        ...
        // Print progress for each tool call
        switch (toolUse.name) {
          case "confirm_understanding":
            console.log(`✅ Confirming understanding...`);
            break;
          case "decide_tech_stack":
            console.log(`🔧 Tech stack: ${input.framework}`);
            break;
          case "plan_site_architecture":
            console.log(`🗺️  Planning architecture...`);
            break;
          case "create_feature_build_list":
            console.log(`📋 Creating feature build list...`);
            break;
          case "write_code":
            console.log(`📝 Writing: ${input.file_path}`);
            filesWritten.push(input.file_path as string);
            break;
          case "flag_unbuildable_feature":
            console.log(`⚠️  Flagging feature: ${input.feature}`);
            break;
        ...
        const result = await handleDeveloperToolCall(
          toolUse.name,
          input,
          outputDir
        );
      ...
      // Once handoff is done, get the final summary
      if (handoffResult) {
